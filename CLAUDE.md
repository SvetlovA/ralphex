...
- `MovePlanToCompleted` commits through `commitFiles` (pathspec-restricted), never the bare `commit`. This call used to run against the user's *main* checkout in worktree mode, which stays usable during the run, so a bare commit swept anything staged there into `move completed plan: ...` (#435); #450 moved the call into the worktree, and the pathspec restriction stays because the worktree index can hold unrelated state too. The source path is passed only when the move went through `git mv`, which stages the source deletion; the `os.Rename` fallback is reached both for an untracked source and for a tracked one whose destination already exists, and naming a path git does not know fails the whole commit. An `os.Lstat(destPath)` immediately before that `os.Rename` refuses the move when `completed/<basename>` is already there: `git mv` declines an existing destination but `os.Rename` replaces it silently, and an uncommitted archive copy has no other copy anywhere. `resolvePlanMoveTargets` therefore returns `done=true` only when the source is gone and a `completed/` copy proves the plan already archived — a source that still exists returns `done=false` whatever sits in `completed/`, so the collision reaches the move path and errors instead of passing for a successful archive (the caller reads `done` as archived). The list is therefore built from the branch taken, not by re-testing the file. Consequence: a pathspec commit is refused while a merge or cherry-pick is in progress in the target checkout (`cannot do a partial commit during a merge`), so the archive fails loudly there instead of silently absorbing the user's merge. The archive is best effort in the caller (`executePlan`), and a failure is reported twice on purpose: as a `WARN:` through the progress logger, so the progress file keeps it where the previous bare stderr write left no trace, and as a `plan archive did not complete: ...` line at the end of the completion summary. A watch-mode dashboard picks that line out of the progress file (`detectEventType`, `pkg/web/tail.go`); a single-session `--serve` run does not, because live SSE events come from `BroadcastLogger` and `web.Logger` carries no `Warn`. A rejected archive commit (e.g. a `commit-msg` hook enforcing conventional commits, #439) leaves the staged rename in the tree, and the `WARN` lands after the runner's `all phases completed successfully` and before the summary block, which is easy to miss. In worktree mode that staged rename is inside the worktree, so the failure also sets the preserve flag that stops cleanup deleting it — see the Worktree Isolation Mode section
...
- `claude_error_patterns` / `codex_error_patterns`: comma-separated error patterns (default strings in the embedded config, mirrored in the README config table; not carried in `llms.txt`). Codex phrases are tightened so review findings that *talk about* rate limiting do not trip a false positive. The claude error set enumerates `API Error:` by code (`400/401/403/404/413/429/500`) rather than the bare substring, keeping failed-run wrapper output specific; clean assistant narration is excluded separately by source provenance. The transient `529/502/503/504` codes are handled by `claude_retry_patterns`, not here
- Matching is case-insensitive substring search
- Whitespace is trimmed from each pattern
- For claude: `parseStream` separates complete surfaced `Output`, bounded recent surfaced `RecentText`, and bounded trusted `DiagnosticText`. The trusted window contains at most 10 structured result/error, assistant API-error, relevant system/error, and non-JSON CLI blocks in chronological order; ordinary assistant content, successful result summaries, and subagent progress are excluded even if they quote a configured phrase. A `result` record counts as diagnostic when it authenticates itself as an error (`is_error:true`, `terminal_reason:"api_error"`, or a non-zero `api_error_status`) — a bare `terminal_reason` does not qualify, because a successful result can carry one. `subtype` is never consulted: Claude Code has been observed emitting `subtype:"success"` on a genuine rate-limit result (anthropics/claude-code#79500). `system/api_retry` is excluded entirely — it reports an attempt, not an outcome, and Claude often recovers from one
- Claude patterns always check `DiagnosticText`. Surfaced `RecentText` is promoted to pattern input only when the process itself failed — a non-zero exit or a stream read error (an idle timeout cancels the stream context, so it arrives as one). Signal absence is deliberately NOT part of that rule: a successful run often emits no marker by design (`review_first.txt` path B stays silent after fixing issues, `task.txt` emits `ALL_TASKS_DONE` only once the plan has no boxes left), so treating "no signal" as failure evidence would readmit narration on clean runs. The promotion path is what keeps limit classification working for the bundled wrapper scripts (gemini/copilot/opencode/pi/agy), whose result records carry no `is_error` field and which surface child stderr as ordinary deltas. The codex-as-claude wrapper discards child stderr and always exits 0 after the child run, so it is not covered. Explicit parent cancellation bypasses pattern checks
- `Result.DiagnosticText` makes the exact classified window available to executor callers and tests; `Result.Output` and normal progress streaming keep their existing full surfaced output behavior. Debug mode additionally labels non-JSON lines as they are classified
...
- Retry detection is suppressed when `result.Signal` is non-empty: a completed run that emitted a structured signal (e.g. `ALL_TASKS_DONE`) must not be discarded and re-run because a trusted diagnostic contains a retry marker. `patternError(patternText, signal)` (`pkg/executor/executor.go`) gates only the retry tier on the signal; trusted limit and error patterns still fire regardless (they surface loudly rather than silently re-running)
...
- `ClaudeExecutor.extractDiagnostic()` classifies trusted stream events; `resolveRunResult()` combines that provenance with signals, idle timeout, and process exit state
